...
import (
	"bytes"
	"encoding"
	"github.com/accessions/core/crc"
	"testing"
)
...
func TestARC(t *testing.T) {
	...
	table := crc.MakeTable(crc.IBM)
	for _, testcase := range tests {
		result := ^crc.Update(0xFFFF, table, testcase.Message)
...
func TestModbus(t *testing.T) {
	...
	for _, testcase := range tests {
		result := ^crc.ChecksumIBM(testcase.Message)
		if testcase.CRC != result {
...
func BenchmarkChecksumIBM(b *testing.B) {
	for i := 0; i < b.N; i++ {
		crc.ChecksumIBM([]byte{0xEA, 0x03, 0x00, 0x00, 0x00, 0x64})
	}
}

func BenchmarkMakeTable(b *testing.B) {
	for i := 0; i < b.N; i++ {
		crc.MakeTable(crc.IBM)
	}
}
...
func TestCCITTFalse(t *testing.T) {
	...
	actual := crc.ChecksumCCITTFalse(data)
	if actual != target {
...
func TestBinaryMarshal(t *testing.T) {
	...
	first := New(crc.IBMTable)
	firstrev := New(crc.MBusTable)
	...
	second := New(crc.IBMTable)
	secondrev := New(crc.MBusTable)
